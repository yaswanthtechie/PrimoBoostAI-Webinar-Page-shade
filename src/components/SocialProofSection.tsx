        ...
        >
          ...
          >
            {[...logos, ...logos].map((logo, index) => (
              <div 
                key={`${logo.name}-${index}`} 
                className="group flex-shrink-0 w-48 h-24 bg-card rounded-2xl border border-white/10 flex items-center justify-center p-4 transition-all duration-300 hover:bg-black/30 hover:border-white/20"
                aria-label={logo.name}
              >
                <img
                  src={logo.url}
                  alt={logo.alt}
                  className="h-8 max-w-full object-contain invert brightness-0 filter group-hover:invert-0 group-hover:brightness-100 transition-all duration-300"
